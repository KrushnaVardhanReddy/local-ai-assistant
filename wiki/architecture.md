...
- Constructing prompts and routing them to the LLM. Includes injecting a **Conversational Context Window** (rolling turn memory, default 3 turns) into the system prompt to allow the LLM to understand contextual follow-up questions. Responses are requested strictly as concise "Talking Points" bullet points to optimize readability during live interviews.
...
- **Web UI & Tunneling**: A Tailwind-styled single-page HTML application (`index.html`) is embedded directly into the Go binary via `//go:embed` and served on the root `/` route of the buddy HTTP server. The app launches the `cloudflared` CLI utility as a subprocess to establish a secure `trycloudflare.com` tunnel pointing to this local server. The tunnel URL is protected by a 16-byte cryptographically random hex token generated per session.
