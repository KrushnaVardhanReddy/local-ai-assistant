...
**Core Stack:**
- **Backend:** Go + `faster-whisper` + ChromaDB + Ollama/OpenAI API.
- **Desktop Client:** Wails v2 + Svelte 5.
...
- **Hardware ID Locking:** Wails generates a `machine_id` saved in the OS keychain to prevent credential sharing on the $49 lifetime license.
...
- **All Core Architecture & Features:** `faster-whisper`, RAG backend (ChromaDB + DuckDuckGo Web Search), Svelte 5 frontend, and Wails OS hooks.
...
3. **Build the Production Bundles:**
   Execute `bash scripts/build.sh` to generate your PyInstaller executables and Wails AppImage/MSI.
4. **Launch!**
