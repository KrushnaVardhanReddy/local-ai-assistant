...
| P40-T4 | `cloud-worker/`, `frontend/src/lib/auth.svelte.ts` | **Cloud User Auth & Session Management:** User signup → API key provisioned in Cloudflare KV → key stored in Tauri `localStorage` → sent as `Authorization: Bearer` on every request. | ⏳ Submitted | Jules |
| P40-T5 | `src-tauri/tauri.cloud.conf.json`, `.env.cloud` | **Tauri Cloud Build Config:** Create cloud-specific Tauri config (no sidecar, product name `BarnOwl Cloud`, new bundle ID) and `.env.cloud` pointing `VITE_API_BASE` at the Worker URL. | ⏳ Submitted | Jules |
