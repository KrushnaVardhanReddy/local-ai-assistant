...
| P40-T8 | `cloud-worker/`, `checkout-server/` | **Stripe Payment Integration (Supabase Edition):** Stripe Checkout for usage-based billing ($2 service fee per interview session + LLM API pass-through). On success, increment `payg_sessions` in Supabase `profiles` table. | ⏳ Submitted | Jules |
