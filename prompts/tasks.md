...
| P40-T6 | `.github/workflows/build-windows-cloud.yml` | **CI/CD Cloud Build Pipeline:** Fast (~2 min) GitHub Actions workflow — no PyInstaller, just `npm build` + `cargo tauri build`. Produces a `~10-15MB` `.msi` installer. | ⏳ Submitted | Jules |
...
| P40-T10 | `cloud-worker/src/index.ts` | **Cloud Worker Backend Auth (Supabase Validation):** Validate the frontend Bearer token against the Supabase `user_api_keys` table and check the `payg_sessions` balance before allowing API access. | ⏳ Submitted | Jules |