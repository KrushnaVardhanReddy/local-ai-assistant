...
| P40-T10 | `cloud-worker/src/index.ts` | **Cloud Worker Backend Auth (Supabase Validation):** Validate the frontend Bearer token against the Supabase `user_api_keys` table and check the `payg_sessions` balance before allowing API access. | ⬜ | — |