...
| Task | Files | Description | Status | PR |
|------|-------|-------------|--------|----|
| P75-T1 | `ScorecardModal.svelte`, `InterviewHUD.svelte`, `prompts.go` | **Post-Interview Analytics & Talking Points:** Build a beautiful Scorecard Modal to visualize the JSON analytics generated by `EndSession()`. Modify the LLM prompt to strictly return 3-5 glanceable "Talking Point" bullets instead of dense paragraphs. | ✅ | #260 |

## Phase 76 — Buddy Mode (Cloudflare Tunnel + Friends Help) 👫
...
| Task | Files | Description | Status | PR |
|------|-------|-------------|--------|----|
| P82-T1 | `ws.svelte.ts`, `Settings.svelte`, `app.go` | **Audio Decoupling:** Decouple `AudioMode` (Speaker vs Dual) from `AppMode` (Meeting vs Transcript). Allows using Dual Mode while the AI Assistant is active. | ✅ | #269 |
