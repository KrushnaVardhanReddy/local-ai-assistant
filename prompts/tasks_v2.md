...
---
| P64-T6 | `wails-app/main.go`, `wails-app/frontend/src/lib/components/AuthModal.svelte` | **Auth Window UX Redesign** — Modify Wails app to start with `AlwaysOnTop: false` and `BackgroundColour` set to a solid color when in Auth mode. Add a custom draggable HTML titlebar to `AuthModal.svelte` with minimize, maximize, and close buttons that call Wails runtime functions. Toggle `AlwaysOnTop` and background color dynamically when authentication succeeds. | ⬜ | — |
