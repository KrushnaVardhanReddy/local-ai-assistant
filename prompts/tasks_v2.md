...
| P73-T4 | `backend/classifier/buffer.go`, `core/engine/engine.go` | **Remove Gemma Classifier:** Remove the llamafile/Gemma-3 local LLM sidecar entirely. Replace `IsQuestionComplete` with an instant always-true function. Delete `gemma.go`, `gemma_test.go`, and `llama_server.go`. Eliminates 5-8s startup, ~600MB RAM, and CPU contention that caused `context deadline exceeded` and dropped questions. | 🔄 | — |
