...
func (p *PresenterApp) PromptOpenDirectory() (node *driving.FileNode, err error) {
	if p.ctx == nil {
		return nil, fmt.Errorf("no valid wails context")
	}
	defer func() {
		if r := recover(); r != nil {
...
func (p *PresenterApp) PromptOpenFile() (doc *driving.WorkspaceDocument, err error) {
	if p.ctx == nil {
		return nil, fmt.Errorf("no valid wails context")
	}
	defer func() {
